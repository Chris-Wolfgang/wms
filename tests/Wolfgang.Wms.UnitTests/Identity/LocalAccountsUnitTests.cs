...
public sealed class LocalAccountsUnitTests
{
    ...
    public void Module_registers_cookie_authentication_the_placeholder_and_its_settings()
    {
        ...
        Assert.Equal(["api.cors.allowed_origins", "auth.integrity.verify_interval", "auth.local.lockout_duration", "auth.local.lockout_threshold", "auth.providers.enabled", "auth.session.idle_timeout", "auth.session.lifetime"], AuthSettings.All.Select(k => k.Name).Order(StringComparer.Ordinal));
